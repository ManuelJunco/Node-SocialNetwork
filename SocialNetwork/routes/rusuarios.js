module.exports = function(app, swig, gestorBD) {
			...
			.post(
					...
					function(req, res) {
						...
						if (req.body.password != req.body.confirmPassword) {
							res
									.redirect("/registrarse?mensaje=La password y su confirmación deben ser iguales&tipoMensaje"
											+ "=alert-danger");
						} else {
							var re = /\S+@\S+/;
							/* Second - Check email */
							if (!re.test(req.body.email)) {
								res
										.redirect("/registrarse?El email no es válido&tipoMensaje=alert-danger");
							} else {
								/* Third - Check password */
								if (req.body.password.length < 7) {
									res
											.redirect("/registrarse?mensaje=Password demasiado corta, minimo 7 caracteres&tipoMensaje"
													+ "=alert-danger");
								} else {
									/* Fourth - Check the name */
									if (req.body.nombre.length < 3) {
										res
												.redirect("/registrarse?mensaje=El nombre es demasiado corto, minimo 3 caracteres&"
														+ "tipoMensaje=alert-danger");
									} else {
										var criterio = {
											email : req.body.email
										...
										gestorBD
												.obtenerUsuarios(
														...
														function(usuarios) {
															if (usuarios == null) {
																res
																		.redirect("/registrarse?mensaje=Error al registrar al usuario&tipoMensaje="
																				+ "alert-danger");
															} else {
																/*
																 * Fifth - Check
																...
																if (usuarios[0] != null) {
																	res
																			.redirect("/registrarse?mensaje=El email ya existe&tipoMensaje=alert-danger");
																} else {
																	/*
																	 * Adding
																	 * user
																	...
																	gestorBD
																			.insertarUsuario(
																					...
																					function(
																						...
																						if (id == null) {
																							res
																									.redirect("/registrarse?mensaje=Error al registrar usuario&tipoMensaje"
																											+ "=alert-danger");
																						} else {
																							res
	...
	app.get("/usuario", function(req, res) {
		...
		gestorBD.obtenerUsuariosPg(criterioCount, criterio, pg, function(
				usuarios, total) {
			if (usuarios == null) {
						...
						gestorBD
								.obtenerPeticiones(
										...
										function(peticiones) {
											if (peticiones == null) {
												res
														.send("Error al enviar petición ");
												/* ¿A DÓNDE DEBERÍA REDIRIGIR? */
											} else {
												if (peticiones[0] != null) {
													if (peticiones[0] == true) {
														res
																.redirect("/usuario?mensaje=Este usuario ya es tu amigo&"
																		+ "tipoMensaje=alert-danger");
													} else {
														res
																.redirect("/usuario?mensaje=Esta petición ya ha sido enviada o quizás deberías"
															...
															.insertarPeticion(
																	...
																	function(id) {
																		if (id == null) {
																			...
																			res
																					.redirect("/usuario?mensaje=Petición enviada con éxito");
																		}
																	});
												}
											}
